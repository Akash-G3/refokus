...
function Navbar() {
  return (
    <div className="max-w-screen-lg mx-auto py-6 flex justify-between border-b-[1px] border-zinc-700">
     ...
     <img
      ...
      <div className="links flex gap-12 ml-20">
        {["Home", "Work", "Culture", "", "News"].map((elem, index) =>
          elem.length === 0 ? (
            <span key={index} className="w-[2px] h-12 bg-zinc-700"></span>
          ) : (
            <a key={index}
            className=" text-white text-sm flex items-center gap-1" href="#">
              {index === 1 && (
                <span
